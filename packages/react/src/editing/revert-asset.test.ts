...
describe('findAssetUsages', () => {
  it('returns 0 when the import is absent', () => {
    const src = [
      "import { ImagePlaceholder } from '@open-slide/react';",
...
describe('applyRevertAsset', () => {
  ...
  it('adds ImagePlaceholder to an existing @open-slide/react named import', () => {
    const src = [
  ...
  it('adds a separate value import when the only @open-slide/react import is type-only', () => {
    ...
    expect(r.source).toContain("import { ImagePlaceholder } from '@open-slide/react';");
  });

  it('adds a fresh @open-slide/react import when none exists', () => {
    const src = [
      "import hero from './assets/hero.png';",
      'export default [() => (',
      "  <img src={hero} alt='x' style={{ objectFit: 'cover' }} />",
    ...
    const r = applyRevertAsset(src, './assets/hero.png');
    if (!r.ok) throw new Error(`expected ok, got ${r.error}`);
    expect(r.source.split('\n')[0]).toBe("import { ImagePlaceholder } from '@open-slide/react';");
  });
