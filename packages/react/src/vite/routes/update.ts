...
//   Compares the running @open-slide/react version against the npm `latest`
...
//   Installs @open-slide/react@latest with the detected package manager, then
//   runs `open-slide sync:skills`.

const PKG = '@open-slide/react';
...
async function updatePackage(ctx: ApiContext): Promise<UpdateResult> {
  ...
  return {
    ...
    latest,
    message: 'Updated @open-slide/react and synced skills.',
  };
}
