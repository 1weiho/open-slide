...
```tsx
import type { DesignSystem, Page } from '@open-slide/react';
...
- `DesignSystem` must be imported from `@open-slide/react` (the panel adds the import automatically when creating a fresh block).
