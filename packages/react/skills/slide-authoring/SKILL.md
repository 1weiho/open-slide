...
- Do not add dependencies. Only `react`, `@open-slide/react`, and standard web APIs are available.
...
When a page genuinely needs a real image **the user has to provide** (product screenshot, team photo, chart from their data), leave a typed `<ImagePlaceholder hint="…" />` from `@open-slide/react` instead of inventing a stand-in — the user replaces it via the Assets panel + inspector. **Do not** use placeholders for decoration or generic stock-photo filler; if type, layout, and color can carry the page, do that.
...
- ❌ Installing packages. Only `react`, `@open-slide/react`, and standard web APIs are available.
