...
```tsx
import { useSlidePageNumber } from '@open-slide/react';
...
- If the theme has runtime-tweakable tokens worth surfacing in the Design panel later, also `export const design: DesignSystem = {...}` (add `import type { DesignSystem } from '@open-slide/react';` alongside the base import).
