...
export async function duplicateSlideDir(
  ...
  try {
    await fs.access(path.join(srcDir, entryFile));
  } catch {
  ...
  const srcEntry = path.join(srcDir, entryFile);
  ...
  try {
    ...
    await fs.writeFile(path.join(dstDir, entryFile), copiedEntrySource, 'utf8');
