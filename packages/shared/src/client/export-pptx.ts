import { isFrameAnimationSettled, waitForDataWaitfor, waitForFonts } from './print-ready.ts';
...
export type PptxFrameCleanup = () => void | Promise<void>;

export type ImagePptxExportOptions = {
  pageCount: number;
  filename: string;
  ...
  renderPage: (
    host: HTMLElement,
    index: number,
  ) => undefined | PptxFrameCleanup | Promise<undefined | PptxFrameCleanup>;
  onProgress?: (progress: PptxExportProgress) => void;
};

export async function exportFramesAsImagePptx(options: ImagePptxExportOptions): Promise<void> {
  if (options.pageCount === 0) return;

  const total = options.pageCount;
  options.onProgress?.({ phase: 'processing', current: 0, total, percent: 0 });
  ...
  const cleanups: PptxFrameCleanup[] = [];
  ...
  for (let i = 0; i < options.pageCount; i++) {
    ...
    if (options.designVars) {
      for (const [key, value] of Object.entries(options.designVars)) {
        host.style.setProperty(key, value);
      }
    }
    container.appendChild(host);
    frames.push(host);
    const cleanup = await options.renderPage(host, i);
  ...
  try {
    ...
    options.onProgress?.({ phase: 'generating', current: total, total, percent: 98 });
    ...
    options.onProgress?.({ phase: 'done', current: total, total, percent: 100 });
