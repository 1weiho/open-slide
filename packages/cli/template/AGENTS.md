...
- **Making images independently editable** — when replacing or cropping one image changes others too (the inspector warns about this), use the `split-shared-images` skill. It moves each `<img>` out of shared components to its own call site.
