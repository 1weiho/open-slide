...
`pnpm dev` boots the dev server on `http://localhost:5173` (or the next free
port) with hot module reload, the inspector, and the assets panel.
...
```text
/create-slide for "Q2 launch — 3 chapters, mixed text density,
                   subtle motion, dark aesthetic"
```
