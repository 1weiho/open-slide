...
workspace can author slides — the community confirms with Claude Code,
Codex, Cursor, Gemini CLI, OpenCode, Windsurf, and Zed. To author cleanly,
the agent needs to:

1. **Read the rules.** `AGENTS.md` (and `CLAUDE.md`) at the workspace root
   define the file contract and constraints.
2. **Find the skills.** The five skills above live under `skills/` (or
   wherever that agent looks for skills).
