...
Vite (which powers both `dev` and `build`) returns asset imports as URL
**strings**. If you reuse slide code under another bundler that returns an
object like `{ src, width, height }`, normalise once:
