...
`/create-slide` offers a theme, asks a few scoping questions, picks a deck
id, plans the structure, and writes pages under `slides/<id>/index.tsx`. See
...
IDs are kebab-case (`q2-launch`, `intro-to-rag`), one folder per deck, and a
deck never touches files outside its own folder — the full workspace rules
live in [`AGENTS.md`](/docs/skills/overview).
