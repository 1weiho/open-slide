...
type SlideTransition = {
  ...
  /** Matches UnstableSharedElement pairs and fades unmatched shared elements. */
...
pages with `UnstableSharedElement` and give both instances the same `id`. The transition
...
content changes, keep that changing copy outside `UnstableSharedElement` and let it fade
