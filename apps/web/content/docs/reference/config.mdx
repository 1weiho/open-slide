...
The runtime UI language is normally picked in-app; `config.locale` only sets
the initial value (and is the place to plug in a custom dictionary).
...
See [Locale](/docs/reference/locale) for the presets and custom dictionaries.
