...
<TypeTable
  type={{
    ...
    themeImport: {
      ...
      default: 'any host allowed',
      description: 'Restrict where themes may be imported from (see below).',
    },
    locale: {
      type: 'Locale',
      deprecated: true,
...
<TypeTable
  type={{
    allowedHosts: {
      type: 'string[]',
      default: 'unset — any host',
      description:
        "Hosts themes may be imported from (exact match or a subdomain of an entry). Leave unset to allow any host — import still warns first, since a theme's demo file is executable code.",
    },
  }}
/>
