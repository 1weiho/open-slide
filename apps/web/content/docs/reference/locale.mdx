...
The runtime ships with four built-in dictionaries. Pick one from the language
picker in the top-right of the home sidebar (next to the theme toggle) — the
choice is saved in the browser and applies across the runtime UI.
...
The `id` field has to be one of the built-in ids
(`'en' | 'zh-TW' | 'zh-CN' | 'ja'`); it drives locale-aware date formatting
and marks the active entry in the language picker, so pick the closest one.
The dictionary keys drive the rest of the UI.
