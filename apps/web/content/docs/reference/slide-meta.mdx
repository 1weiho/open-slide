...
its container; the runtime renders one at a time onto the workspace's configured
canvas: 1920×1080 by default, a native 4K / 8K preset, or explicit dimensions.
...
In a Vite-powered slide project, `CANVAS_WIDTH` and `CANVAS_HEIGHT` expose the
workspace's exact configured dimensions.
