...
<Callout type="warn">
  The shared element API is currently exported as `UnstableSharedElement`.
  Treat it as experimental and subject to change.
</Callout>

`ShareElement` is the conceptual primitive for making one visual object move
between two pages. In code today, import `UnstableSharedElement` and alias it
to the component name you want to use.

```tsx title="slides/magic-move/index.tsx"
import {
  UnstableSharedElement as ShareElement,
  type Page,
  type SlideTransition,
} from '@open-slide/core';

const Token = ({ x, y, size }: { x: number; y: number; size: number }) => (
  <ShareElement id="token">
    <div
      style={{
        position: 'absolute',
        left: x,
...
`UnstableSharedElement` accepts one child. If the child is a DOM element, the
runtime writes the `data-osd-shared-element` marker onto that child. Otherwise
