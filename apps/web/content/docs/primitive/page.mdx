---
title: Page
description: The component contract for one configured-canvas slide page.
---
...
- The root element should fill the canvas with `width: '100%'` and
  `height: '100%'`.
- Every page renders into the configured canvas: `1920 × 1080` by default,
  a 4K / 8K preset, or explicit dimensions. The runtime scales the whole canvas
  uniformly for preview and present mode; exports retain the configured size.

## Layout model

Write pages as fixed-format compositions, not responsive web pages. Pixel
coordinates are stable within the configured canvas. Use absolute positioning
when exact placement matters, and use flex/grid when a section needs internal
rhythm.
